	    ...
	    if [ ! -d "$tempFolder" ]; then
	      mkdir -p "$tempFolder" 
            ...
            if [ ! -z "$cookiesFile" ]; then
                yt-dlp -f "$videoFormat" --no-video-multistreams --cookies "$cookiesFile" -o "$tempFolder/$finalFileName" --write-auto-subs --write-subs --sub-langs "en" --sub-format srt --merge-output-format mkv --sponsorblock-mark all --sponsorblock-remove sponsor --no-mtime --geo-bypass $ytdlpExtraOpts "https://www.youtube.com/watch?v=$tmdbExtraKey"  2>&1 | tee -a /config/logs/$scriptName.txt
            else
                yt-dlp -f "$videoFormat" --no-video-multistreams -o "$tempFolder/$finalFileName" --write-auto-subs --write-subs --sub-langs "en" --sub-format srt --merge-output-format mkv --sponsorblock-mark all --sponsorblock-remove sponsor --no-mtime --geo-bypass $ytdlpExtraOpts "https://www.youtube.com/watch?v=$tmdbExtraKey"  2>&1 | tee -a /config/logs/$scriptName.txt
            fi
